...
export class MyFavouriteDisplayItem extends React.Component<
  IMyFavouriteDisplayItemProps,
  IMyFavouriteDisplayItemState
> {
  ...
  public render(): React.ReactElement<IMyFavouriteDisplayItemProps> {
    const firstLetter: string | undefined =
      this.props.displayItem.Title?.charAt(0).toUpperCase();
    return (
      <div className={`${styles.ccitemContent}`}>
        ...
        <div className={styles.ccitemDesc}>
          ...
          <PrimaryButton
            ...
            className={styles.ccButton}
          />
          <div className={styles.ccStatus}>{this.state.status}</div>
        </div>
      </div>
    );
  }
  ...
  private async _deleteFavourite(): Promise<void> {
    ...
    await this.props.deleteFavourite(Number(this.props.displayItem.Id));
    status = <span></span>;
    disableButtons = false;
