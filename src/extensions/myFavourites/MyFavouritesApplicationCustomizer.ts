...
export default class MyFavouritesApplicationCustomizer extends BaseApplicationCustomizer<IMyFavouritesApplicationCustomizerProperties> {
  @override
  public onInit(): Promise<void> {
    let placeholder: PlaceholderContent | undefined;
    placeholder = this.context.placeholderProvider.tryCreateContent(
      PlaceholderName.Top
    );

    // init the react top bar component.
    const element: React.ReactElement<IMyFavouritesTopBarProps> =
      React.createElement(MyFavouritesTopBar, {
        context: this.context,
      });

    // render the react element in the top placeholder.
    ReactDOM.render([element], placeholder?.domElement || document.body);
